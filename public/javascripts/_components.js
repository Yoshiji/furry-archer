Crafty.c("Tile", {
  init: function () {
    ...
    this.addComponent("Collision").collision(
      new Crafty.polygon([0, 84], [128, 20], [256, 84], [128, 148])
    );
...
Crafty.c("Player", {
  init: function () {
    ...
    this._id = null;
    this.addComponent(
      "2D, Canvas, player2, SpriteAnimation, Fourway, SolidHitBox, Collision, Socketed"
    );
    ...
    this.bind("NewDirection", function (direction) {
      if (direction.x < 0) {
        ...
        if (!this.isPlaying("walk_up")) this.stop().animate("walk_up", 1, -1);
      } else if (direction.y > 0) {
    ...
    this.bind("WalkingOnNewTile", function (tile) {
      var tile_settings = map.get_tile_settings(
        { x: tile._x, y: tile._y },
        true
      );
      var data = {
        user_id: user._id,
        pos_x: tile_settings.x,
        pos_y: tile_settings.y,
      };
  ...
  update_several_infos: function (tile_settings) {
    $(".value.position")
      .empty()
      .append(tile_settings.x + " : " + tile_settings.y);
  ...
  reload_area: function () {
    ...
    if (
      area.x.start != new_area.x.start ||
      area.x.end != new_area.x.end ||
      area.y.start != new_area.y.start ||
      area.y.end != new_area.y.end
    ) {
      area = new_area;
