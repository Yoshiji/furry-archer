module.exports = function (mongoose) {
  ...
  TileSchema.methods.harvest_and_sell = function (
    user_id,
    tile,
    cb_update_infos,
    cb_update_tile
    ...
    Crop.findOne({ _id: tile.crop[0] }, function (err, crop) {
      ...
      if (crop.maturity > 80) {
        ...
        console.log(
          "Harvesting a mature plant for GOLD: " + gold_gained.toString()
        );
  ...
  TileSchema.methods.harvest_and_store = function (
    user_id,
    tile,
    cb_update_infos,
    cb_update_tile
  ...
  TileSchema.statics.raise_fertility_routine = function () {
    ...
    Tile.find({ crop: { $size: 0 } }, function (err, tiles) {
      tiles.forEach(function (tile) {
        if (tile.fertility < 100) tile.fertility += 1;
        tile.save();
      });
    });
  };
  ...
  TileSchema.statics.raise_humidity_routine = function () {
    ...
    Tile.find({}, function (err, tiles) {
      tiles.forEach(function (tile) {
        if (tile.humidity < 100) tile.humidity += 1;
        tile.save();
      });
    });
  };
  ...
  TileSchema.statics.generate = function () {
    ...
    for (var i = 0; i < map_size; i++) {
      ...
      for (var j = 0; j < map_size; j++) {
        ...
        var fertility_rand =
          Math.floor(Math.random() * (current_band[1] - current_band[0])) +
          current_band[0];
