          ...
          switch (building_template.tile_size) {
            case 1:
              User.check_can_afford(
                ...
                building_template.price,
                function (user) {
                  socket.emit("update_infos", user);
                  Building.create(
                    {
                      name: building_template.name,
                      storage: building_template.storage,
                      consommation: building_template.consommation,
                      user: socket.session.user._id,
              ...
              Tile.find(
                {
                  $or: [
                    { x: { $in: in_x }, y: { $in: in_y } },
                    { x: { $in: in_x2 }, y: { $in: in_y2 } },
                  ],
                  owner_name: socket.session.user.username,
                },
                function (err, tiles) {
                  if (tiles.length > 3) {
                    console.log(coords);
