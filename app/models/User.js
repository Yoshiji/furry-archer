module.exports = function (mongoose) {
  ...
  UserSchema.statics.reload_stock = function (user_id, socket) {
    User.findOne({ _id: user_id }, function (err, user) {
      if (!user || err) {
        console.log(err, "ERROR check can store");
        return;
      }
      ...
      Building.find({ user: user_id }, function (err, buildings) {
        ...
        StoredCrop.find(
          { user: user_id, dead_at: { $gt: new Date() } },
          function (err, crops) {
            if (err) {
              console.log(err);
            }
  ...
  UserSchema.statics.check_can_afford = function (
    user_id,
    gold_amount,
    callback
  ) {
    User.findOne({ _id: user_id }, function (err, user) {
      if (!user || err) {
        console.log(err, "ERROR check can afford");
        return;
      }
  ...
  UserSchema.statics.sell_stored_crops = function (user_id, callback) {
    User.findOne({ _id: user_id }, function (err, user) {
      if (!user || err) {
        console.log(err, "ERROR sell stored crops");
        return;
      }
