...
UTILS = {
  ...
  Routines: {
    connection: function (socket) {
      ...
      UTILS.Chat.broadcast(
        socket,
        socket.session.user.username + " connected!"
      );
  ...
  Socket: {
    by_username: function (sockets, username) {
      for (
        var i = 0, client_length = sockets.clients().length;
        i < client_length;
        i++
      ) {
        if (
          sockets.clients()[i] &&
          sockets.clients()[i].session &&
          sockets.clients()[i].session.user.username == username
        ) {
    ...
    by_user_id: function (sockets, user_id) {
      for (
        ...
        if (
          sockets.clients()[i] &&
          sockets.clients()[i].session &&
          sockets.clients()[i].session.user._id == user_id
        ) {
          return sockets.clients()[i];
          break;
        }
  ...
  Timeouts: {
    ...
    reload_stock_value: function (io) {
      ...
      if (typeof io.sockets != "undefined")
        io.sockets.emit("update_stock_value", {
          value: UTILS.Timeouts.CURRENT_STOCK_VALUE,
        });
      else
        io.emit("update_stock_value", {
          value: UTILS.Timeouts.CURRENT_STOCK_VALUE,
        });
    },
  ...
  Map: {
    deploy_settings: function () {
      ...
      Settings.findOne({}, function (err, settings) {
        if (settings) {
          ...
          var default_values = {
            ratio_level_tile: 0.5,
            offset_level_tile: 10,
            cycle_duration: 60,
            tiles_to_next_level: 10,
            gold_amount_at_starting: 20,
          };
    ...
    generate: function () {
      ...
      CropTemplate.find(function (err, crop_templates) {
        if (crop_templates.length < 1) CropTemplate.generate();
      });

      BuildingTemplate.find(function (err, building_templates) {
        if (building_templates.length < 1) BuildingTemplate.generate();
      });

      WeaponTemplate.find(function (err, weapon_templates) {
        if (weapon_templates.length < 1) WeaponTemplate.generate();
      });

      Tile.find(function (err, tiles) {
        if (tiles.length < 1) Tile.generate();
      });
    },
    ...
    update_actions: function (socket, tile) {
      ...
      if (tile && tile.owner_name == socket.session.user.username) {
        ...
        if (tile.crop.length > 0) {
          ...
          if (tile.crop[0].maturity > 80) level = 2;
          else level = 1;
        } else if (tile.building.length > 0) {
          level = 1;
        } else {
          // Si c'est notre tile mais n'a pas de Crop
          level = 0;
        }
      }
      ...
      if (level == 0) {
          ...
          if (crop_templates.length > 0) {
            for (var i = 0, length = crop_templates.length; i < length; i++) {
              available_actions.push([
                "Plant " + crop_templates[i].name,
                crop_templates[i].seed_price,
              ]);
            }
          }
          ...
          BuildingTemplate.find({}, function (err, building_templates) {
            if (building_templates.length > 0) {
              for (var i = 0; i < building_templates.length; i++) {
                available_actions.push([
                  "Build " + building_templates[i].name,
                  building_templates[i].price,
                ]);
              }
            }
        ...
        if (
          tile &&
          socket &&
          socket.session.user.username != tile.owner_name &&
          tile.owner_name != -1 &&
          !tile.is_attacked
        ) {
    ...
    update_weapons: function (socket) {
      ...
      WeaponTemplate.find({}, function (err, weapon_templates) {
        if (weapon_templates.length > 0) {
          User.findOne({ _id: socket.session.user._id })
            .populate("weapons")
            .exec(function (err, user) {
              for (
                var i = 0, wt_length = weapon_templates.length;
                i < wt_length;
                i++
              ) {
                ...
                if (user.weapons && user.weapons.length > 0) {
                  for (
                    var j = 0, length = user.weapons.length;
                    j < length;
                    j++
                  ) {
                ...
                if (already_bought)
                  if (already_bought.is_in_use == true)
                    weapons_actions.push([
                      "Using " + weapon_templates[i].name,
                      false,
                      already_bought._id,
                      already_bought.is_in_use,
                    ]);
                  else
                    weapons_actions.push([
                      "Use " + weapon_templates[i].name,
                      false,
                      already_bought._id,
                      already_bought.is_in_use,
                    ]);
                  ...
                  weapons_actions.push([
                    "Buy " + weapon_templates[i].name,
                    weapon_templates[i].price,
                    weapon_templates[i]._id,
                  ]);
              }
    ...
    buy_weapon: function (weapon_id, socket) {
      WeaponTemplate.findOne(
        { _id: weapon_id },
        function (err, weapon_template) {
          if (weapon_template) {
            User.check_can_afford(
              socket.session.user._id,
              weapon_template.price,
              function (user) {
                ...
                weapon.save(function (err) {
                  ...
                  if (user.weapons) user.weapons.push(weapon._id);
                  else user.weapons = weapon._id;
    ...
    plant: function (tile, crop_name, socket) {
      if (tile.crop.length < 1 && tile.building.length < 1) {
        CropTemplate.findOne(
          { name: crop_name },
          function (err, crop_template) {
            User.check_can_afford(
              socket.session.user._id,
              crop_template.seed_price,
              function (user) {
                socket.emit("update_infos", user);
                ...
                var maturity_interval = setInterval(
                  crop.reload_maturity_routine,
                  crop_template.maturation_time * 100,
                  crop,
                  function (tile) {
                    UTILS.Map.update_tile(socket, tile);
                  },
    ...
    attack: function (tile, socket) {
      User.findOne({ _id: socket.session.user._id }, function (err, user) {
        ...
        Weapon.findOne(
          { _id: { $in: user.weapons }, is_in_use: true },
          function (err, weapon) {
            if (err) console.log(err);
            ...
            if (weapon) {
              User.update(
                ...
                { $set: { is_fighting: true } },
                function (err, user_target) {
                  if (err) console.log(err);
                }
              );
              ...
              tile.save(function (err) {
                ...
                if (UTILS.Socket.by_username(io.sockets, tile.owner_name))
                  UTILS.Socket.by_username(io.sockets, tile.owner_name).emit(
                    "attack_alert",
                    {
                      attacker: socket.session.user.username,
                      x: tile.x,
                      y: tile.y,
                    }
                  );
            ...
            if (weapon) {
              if (weapon.hit_ratio < Math.floor(Math.random() * 100 + 1)) {
                ...
                if (
                  socket.session.user.username == tile.owner_name &&
                  tile.attacked_by &&
                  tile.attacked_by != "false"
                ) {
                  console.log("CHATTE", tile.attacked_by);
                  User.findOne(
                    { username: tile.attacked_by },
                    function (err, user_to_attack) {
                      user_to_attack.health -= health_lost;
                      if (user_to_attack.health < 0) user_to_attack.health = 0;
                      user_to_attack.save(function (err) {
                        ...
                        if (user_to_attack.health <= 0) {
                          ...
                          if (
                            UTILS.Socket.by_username(
                              io.sockets,
                              user_to_attack.username
                            )
                          )
                            UTILS.Socket.by_username(
                              io.sockets,
                              user_to_attack.username
                            ).emit("update_infos", user_to_attack);
                        }
                      });
                    }
                  );
    ...
    end_fight: function (tile, user, user_to_attack, socket) {
      Tile.find(
        { owner_name: user_to_attack.username, is_attacked: true },
        function (err, tiles) {
          if (err) console.log(err);
...
io.set("authorization", function (data, accept) {
  cookieParser(data, {}, function (err) {
    if (err) {
      ...
      session_store.get(
        data.signedCookies["connect.sid"],
...
PlayerList = (function () {
  function PlayerList() {
    ...
    for (var i = 0, len = io.sockets.clients().length; i < len; i++) {
      ...
      if (client.session.user == undefined) {
        client.session.user = { username: "Toby", id: "0" };
      }
      var player = {
        socketID: client.id,
